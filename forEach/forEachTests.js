...
try {
    ...
    console.log('ALL TESTS PASSED, WOOT')
}
catch(err) {
    console.log(err);
}
...
function forEach(array, callbackFunction, optionalThis) {
    ...
    //if optional This arg is present, bind it to the callback function
    if(optionalThis) {
        innerCallbackFunction = callbackFunction.bind(optionalThis);
    }
    
    for(var i=0; i<array.length; i++) {
        innerCallbackFunction(array[i], i, array);
    }
}